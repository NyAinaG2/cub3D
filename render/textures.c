...
/*   Updated: 2026/05/19 12:35:47 by mrakotos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_column(float len, int x, t_test *data, t_params *params)
{
	int			start;
	int			end;
	int			start_unclipped;
	...
	column.len = roundf(len);
	start_unclipped = (WIN_H - len) / 2;
	start = start_unclipped;
	end = start_unclipped + len - 1;
	if (start < 0)
		start = 0;
	if (end >= WIN_H)
		end = WIN_H - 1;
